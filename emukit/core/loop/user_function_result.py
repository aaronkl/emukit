...
class UserFunctionResult(object):
    ...
    def __init__(self, X: np.ndarray, Y: np.ndarray, C: np.ndarray = None) -> None:
        ...
        :param C: Cost of evaluating the function, 1 by function cost dimension
        """
        if X.ndim != 1:
        ...
        if C is not None and C.ndim != 1:
            raise ValueError("y is expected to be 1-dimensional, actual dimensionality is {}".format(C.ndim))
